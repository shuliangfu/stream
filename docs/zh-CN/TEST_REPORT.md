...
- ✅ **错误处理**
  - 不支持适配器类型的错误处理
- ✅ **列表过滤**
  - `listStreams` 支持 filter（name、roomId、status、protocol）
  - `listRooms` 支持 filter（name、isPrivate）
- ✅ **适配器**
  - nginx-rtmp 适配器（mock stat）
  - livekit 适配器（mock API）
- ✅ **ServiceContainer 集成** (6 个测试)
  - 获取默认/自定义管理器名称
  - 设置和获取服务容器
  - 从服务容器获取 StreamManager
  - 服务不存在时返回 undefined
  - 支持多个 StreamManager 实例
...
- ✅ **SRS 适配器**（mock fetch，无需真实 SRS 服务器）
  - 创建适配器实例、默认配置
  - 创建流、获取流、不存在的流返回 null
  - 列出所有流、分页（limit/offset）
  - 删除流、获取统计信息
...
- ✅ **NginxRtmpAdapter**（mock fetch /stat）
  - 创建适配器实例、默认配置
  - createStream 登记流并返回推拉 URL
  - getStream 从 stat 同步在播流、本地已登记流
...
- ✅ **Stream 客户端 - 浏览器测试**（@dreamer/test 浏览器集成，Puppeteer +
  esbuild）
  - StreamClient 挂载及 ClientPublisher、ClientSubscriber 导出
  - ClientPublisher 实例与方法（connect、publish、stop、on、off、getStatistics）
  - ClientSubscriber
    实例与方法（connect、subscribe、stop、play、pause、on、off、getStatistics）
...
- ✅ **ServerPublisher**
  ...
  - 未连接时 publish 抛出、重复 connect 抛出、on/off 事件
  - getHlsPlaylistPath 在非 HLS 推流时返回 undefined
  - publish 使用 WebRTC URL 或非 Blob/非 string 媒体源时抛出明确错误
- ✅ **ServerSubscriber**
  - 创建实例、getStatistics（idle）、connect 后状态为 connected
...
- ✅ 获取统计信息
- ✅ 对不支持的适配器类型抛出错误
- ✅ 列表过滤（listStreams / listRooms filter）
- ✅ 适配器（nginx-rtmp、livekit mock）
- ✅ ServiceContainer 集成（6 个测试）
- ✅ createStreamManager 工厂函数（5 个测试）
...
- ✅ **核心功能**: 流管理、房间管理、适配器
- ✅ **集成测试**: 流生命周期、视频文件推流
- ✅ **工具函数**: 错误处理、协议处理、URL 生成、状态管理、ID
  生成、缓存、队列、连接池、重连管理、批量操作
- ✅ **错误处理**: 所有错误类型都有测试覆盖
...
- ✅ 批量操作（批量创建、删除、获取）
- ✅ 视频文件推流（循环播放、质量设置）
- ✅ ServiceContainer 集成（管理器名称、容器设置/获取、fromContainer）
- ✅ createStreamManager 工厂函数
